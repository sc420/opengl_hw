...
class SceneContext {
 public:
  ...
  const FbxArray<FbxString *> &GetAnimStackNameArray() const {
    return mAnimStackNameArray;
  }
  // Get all the pose in current scene.
  const FbxArray<FbxPose *> &GetPoseArray() const { return mPoseArray; }

  // The input index is corresponding to the array returned from
  // GetAnimStackNameArray.
  ...
  enum CameraZoomMode { ZOOM_FOCAL_LENGTH, ZOOM_POSITION };
