...
//      vec2 kernel =
//          vec2(exp(-d * d / twoSigmaESquared), exp(-d * d /
//          twoSigmaRSquared));
...
//}
//
// vec3 posterize(vec3 color, float steps) { return floor(color * steps) /
// steps; }
//
...
//                           ? r1
//                           : 0.9999999));  // note: weird ass bug on old
//                           drivers
...
vec4 CalcOriginal() {
  const vec4 color = GetTexel(original_tex, vs_tex_coords);
...
vec4 CalcPostproc() {
  ...
  return GetTexel(hdr_tex, vs_tex_coords);
}

vec4 CalcInteractivePostproc() {
