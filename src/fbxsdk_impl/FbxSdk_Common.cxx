...
void DestroySdkObjects(FbxManager* pManager, bool pExitStatus) {
  // Delete the FBX Manager. All the objects that have been allocated using the
  // FBX Manager and that haven't been explicitly destroyed are also
...
bool SaveScene(FbxManager* pManager, FbxDocument* pScene, const char* pFilename,
  ...
  if (pFileFormat < 0 ||
      pFileFormat >= pManager->GetIOPluginRegistry()->GetWriterFormatCount()) {
    ...
    for (lFormatIndex = 0; lFormatIndex < lFormatCount; lFormatIndex++) {
      if (pManager->GetIOPluginRegistry()->WriterIsFBX(lFormatIndex)) {
        FbxString lDesc =
            pManager->GetIOPluginRegistry()->GetWriterFormatDescription(
                lFormatIndex);
...
bool LoadScene(FbxManager* pManager, FbxDocument* pScene,
  ...
  if (lImporter->IsFBX()) {
    ...
    for (i = 0; i < lAnimStackCount; i++) {
      ...
      // Change the value of the import name if the animation stack should be
