...
void GlSetCameraPerspective(double pFieldOfViewY, double pAspect,
                            double pNearPlane, double pFarPlane,
                            FbxVector4& pEye, FbxVector4& pCenter,
                            FbxVector4& pUp, double pFilmOffsetX,
                            double pFilmOffsetY) {
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glTranslated(pFilmOffsetX, pFilmOffsetY, 0);
  gluPerspective(pFieldOfViewY, pAspect, pNearPlane, pFarPlane);
...
void GlSetCameraOrthogonal(double pLeftPlane, double pRightPlane,
                           double pBottomPlane, double pTopPlane,
                           double pNearPlane, double pFarPlane,
                           FbxVector4& pEye, FbxVector4& pCenter,
                           FbxVector4& pUp) {
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glOrtho(pLeftPlane, pRightPlane, pBottomPlane, pTopPlane, pNearPlane,
          pFarPlane);
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  gluLookAt(pEye[0], pEye[1], pEye[2], pCenter[0], pCenter[1], pCenter[2],
...
void GlDrawCamera(FbxAMatrix& pGlobalPosition, double pRoll) {
  ...
  int i;
  float lCamera[10][2] = {{0, 5.5},    {-3, 4.5},   {-3, 7.5},   {-6, 10.5},
                          {-23, 10.5}, {-23, -4.5}, {-20, -7.5}, {-3, -7.5},
                          {-3, -4.5},  {0, -5.5}};
...
void GlDrawCrossHair(FbxAMatrix& pGlobalPosition) {
  ...
  double lCrossHair[6][3] = {{-3, 0, 0}, {3, 0, 0},  {0, -3, 0},
                             {0, 3, 0},  {0, 0, -3}, {0, 0, 3}};
