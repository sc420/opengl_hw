...
#include "fbxsdk_impl/GlFunctions.h"
#include "fbxsdk_impl/SceneContext.h"
#include "fbxsdk_impl/SetCamera.h"

#define HFOV2VFOV(h, ar)                                 \
  (2.0 * atan((ar)*tan((h * FBXSDK_PI_DIV_180) * 0.5)) * \
   FBXSDK_180_DIV_PI)  // ar : aspectY / aspectX
#define VFOV2HFOV(v, ar)                                 \
  (2.0 * atan((ar)*tan((v * FBXSDK_PI_DIV_180) * 0.5)) * \
   FBXSDK_180_DIV_PI)  // ar : aspectX / aspectY

FbxCamera* GetCurrentCamera(FbxScene* pScene, FbxTime& pTime,
                            FbxAnimLayer* pAnimLayer,
                            const FbxArray<FbxNode*>& pCameraArray);
void GetCameraAnimatedParameters(FbxNode* pNode, FbxTime& pTime,
                                 FbxAnimLayer* pAnimLayer);
...
void SetCamera(FbxScene* pScene, FbxTime& pTime, FbxAnimLayer* pAnimLayer,
               const FbxArray<FbxNode*>& pCameraArray, int pWindowWidth,
               int pWindowHeight) {
  // Find the current camera at the given time.
  FbxCamera* lCamera =
      GetCurrentCamera(pScene, pTime, pAnimLayer, pCameraArray);
  if (lCamera == NULL) return;
  ...
  if (lCameraNode && lCameraNode->GetTarget()) {
    ...
    if (!lCameraNode || IsProducerCamera(pScene, lCamera)) {
      if (lCamera) lCenter = lCamera->InterestPosition.Get();
    } else {
      // Get the direction
      ...
      FbxVector4 lCameraGlobalPosition(
          GetGlobalPosition(lCameraNode, pTime).GetT());
      double lLength =
          (FbxVector4(lInterestPosition - lCameraGlobalPosition).Length());

      // Set the center.
      // A camera with rotation = {0,0,0} points to the X direction. So create a
      // vector in the X direction, rotate that vector by the global rotation
      // amount and then position the center by scaling and translating the
      // resulting vector
  ...
  if (lCamera) lRadians = lCamera->Roll.Get() * FBXSDK_PI_DIV_180;
  lUp = lUp * cos(lRadians) + lRight * sin(lRadians);

  double lNearPlane = 0.01;
  if (lCamera) lNearPlane = lCamera->GetNearPlane();
  double lFarPlane = 4000.0;
  if (lCamera) lFarPlane = lCamera->GetFarPlane();
  ...
  if (lCamera && lCamera->ProjectionType.Get() == FbxCamera::ePerspective) {
    ...
    FbxCamera::EAspectRatioMode lCamAspectRatioMode =
        lCamera->GetAspectRatioMode();
    ...
    double lFilmWidth =
        lCamera->GetApertureWidth() * lCamera->GetSqueezeRatio();
    ...
    if (lCamera->GetApertureMode() == FbxCamera::eVertical) {
      ...
      lFieldOfViewX =
          lCamera->ComputeFieldOfView(lCamera->FocalLength.Get());  // get HFOV
      lFieldOfViewY = HFOV2VFOV(lFieldOfViewX, lApertureRatio);
    ...
    int lViewPortPosX = 0, lViewPortPosY = 0, lViewPortSizeX = pWindowWidth,
    ...
    GlSetCameraPerspective(lFieldOfViewY, lAspectRatio, lNearPlane, lFarPlane,
  ...
  // Get the relevant camera settings for an orthogonal view.
    ...
    if (lCamera) lPixelRatio = lCamera->GetPixelRatio();
    ...
    GlSetCameraOrthogonal(lLeftPlane, lRightPlane, lBottomPlane, lTopPlane,
                          lNearPlane, lFarPlane, lEye, lCenter, lUp);
  }
}
...
FbxCamera* GetCurrentCamera(FbxScene* pScene, FbxTime& pTime,
  ...
  if (lCurrentCameraName.Compare(FBXSDK_CAMERA_PERSPECTIVE) == 0) {
    ...
    FbxCameraSwitcher* lCameraSwitcher =
        pScene->GlobalCameraSettings().GetCameraSwitcher();
...
void GetCameraAnimatedParameters(FbxNode* pNode, FbxTime& pTime,
  ...
  if (fc) lCamera->Roll.Set(fc->Evaluate(pTime));
  ...
  if (lCameraApertureMode == FbxCamera::eHorizontal ||
    ...
    if (fc) lFieldOfView = fc->Evaluate(pTime);
    ...
    if (fc) lNewFieldOfViewX = fc->Evaluate(pTime);

    fc = lCamera->FieldOfViewY.GetCurve(pAnimLayer);
    if (fc) lNewFieldOfViewY = fc->Evaluate(pTime);
    ...
    lUpdatedApertureX *= tan(lNewFieldOfViewX * 0.5 * FBXSDK_PI_DIV_180) /
                         tan(lOldFieldOfViewX * 0.5 * FBXSDK_PI_DIV_180);
    lUpdatedApertureY *= tan(lNewFieldOfViewY * 0.5 * FBXSDK_PI_DIV_180) /
                         tan(lOldFieldOfViewY * 0.5 * FBXSDK_PI_DIV_180);
    ...
    if (fc && fc->Evaluate(pTime)) lFocalLength = fc->Evaluate(pTime);
...
bool IsProducerCamera(FbxScene* pScene, FbxCamera* pCamera) {
  ...
  if (pCamera == lGlobalCameraSettings.GetCameraProducerPerspective())
    return true;
  if (pCamera == lGlobalCameraSettings.GetCameraProducerTop()) return true;
  if (pCamera == lGlobalCameraSettings.GetCameraProducerBottom()) return true;
  if (pCamera == lGlobalCameraSettings.GetCameraProducerFront()) return true;
  if (pCamera == lGlobalCameraSettings.GetCameraProducerBack()) return true;
  if (pCamera == lGlobalCameraSettings.GetCameraProducerRight()) return true;
  if (pCamera == lGlobalCameraSettings.GetCameraProducerLeft()) return true;
...
void UpdatePerspCameraAttributes(FbxCamera* pCamera, double pNewApertureW,
                                 double pNewApertureH) {
  if (pCamera == NULL || FbxAnimUtilities::IsAnimated(pCamera)) return;
  ...
  if (pCamera->GetApertureMode() == FbxCamera::eFocalLength) {
    ...
    double lFOVOrig =
        pCamera->ComputeFieldOfView(lFLOrig);  // recompute to be sure.
    ...
    double lFLOrig =
        pCamera->ComputeFocalLength(lFOVOrig);  // recompute to be sure.
    // transform aperture width and height temporarily
    pCamera->SetApertureWidth(pNewApertureW);
    pCamera->SetApertureHeight(pNewApertureH);
    ...
    double lFLOrig =
        pCamera->ComputeFocalLength(lFOVOrigX);  // recompute to be sure.
    // transform aperture width and height temporarily
    pCamera->SetApertureWidth(pNewApertureW);
    pCamera->SetApertureHeight(pNewApertureH);
  ...
  if (pZoomMode == SceneContext::ZOOM_FOCAL_LENGTH) {
    ...
    if (lCameraNode && lCameraNode->GetTarget()) {
      ...
      if (!lCameraNode || IsProducerCamera(pScene, lCamera)) {
        ...
        // Set the center.
        // A camera with rotation = {0,0,0} points to the X direction. So create
        // a vector in the X direction, rotate that vector by the global
        // rotation amount and then position the center by scaling and
        // translating the resulting vector
        lRotationVector = FbxVector4(1.0, 0, 0);
...
void CameraOrbit(FbxScene* pScene, FbxVector4 lOrigCamPos, double OrigRoll,
                 int dX, int dY) {
  ...
  if (lNewPosition[2] == 0) {
    ...
    rotX = (int)(atan((double)lNewPosition[0] / (double)lNewPosition[2]) *
                 FBXSDK_180_DIV_PI);
  }
  bool bRoll = (((int)OrigRoll % 360) != 0);
  if ((lNewPosition[2] < 0 && !bRoll) || (lNewPosition[2] > 0 && bRoll)) {
    dY = -dY;
  }
  if (bRoll) dX = -dX;
...
void CameraPan(FbxScene* pScene, FbxVector4 lOrigCamPos,
               FbxVector4 lOrigCamCenter, double OrigRoll, int dX, int dY) {
  ...
  FbxAMatrix lRotationX, lRotationY, lRotationXInverse, lRotationYInverse,
      lTranslation;
  ...
  if (lCamera == lGlobalCameraSettings.GetCameraProducerTop()) {
    ...
    double dist = (double)(lDist[0] * lDist[0] + lDist[1] * lDist[1] +
                           lDist[2] * lDist[2]);
    // aZ
    if (lDist[2] == 0) {
      aZ = 90.0;
    ...
    // aY
    if (dist > 0.001) {
      aY = (asin(sqrt((double)(lDist[1] * lDist[1]) / dist)) *
            FBXSDK_180_DIV_PI);
    } else {
      aY = 0;
    }
    ...
    lTranslation = lRotationYInverse * lRotationXInverse * lTranslation *
                   lRotationY * lRotationX;
