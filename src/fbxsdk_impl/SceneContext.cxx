...
#include "fbxsdk_impl/DrawText.h"
#include "fbxsdk_impl/FbxSdk_Common.h"
#include "fbxsdk_impl/SceneCache.h"
#include "fbxsdk_impl/SetCamera.h"
#include "fbxsdk_impl/targa.h"
...
void PreparePointCacheData(FbxScene* pScene, FbxTime& pCache_Start,
                           FbxTime& pCache_Stop) {
  ...
  for (int lIndex = 0; lIndex < lNodeCount; lIndex++) {
    ...
    if (lNode->GetGeometry()) {
      int i,
          lVertexCacheDeformerCount =
              lNode->GetGeometry()->GetDeformerCount(FbxDeformer::eVertexCache);
      ...
      for (i = 0; i < lVertexCacheDeformerCount; ++i) {
        ...
        FbxVertexCacheDeformer* lDeformer =
            static_cast<FbxVertexCacheDeformer*>(
                lNode->GetGeometry()->GetDeformer(i,
        ...
        if (lDeformer->Active.Get()) {
          if (lCache->GetCacheFileFormat() == FbxCache::eMaxPointCacheV2) {
            ...
            // turn it on if you need it.
            //#if 0
            if (!lCache->ConvertFromMCToPC2(
                    FbxTime::GetFrameRate(
                        pScene->GetGlobalSettings().GetTimeMode()),
                    0, &lStatus)) {
              // Conversion failed, retrieve the error here
          ...
          if (!lCache->OpenFileForRead(&lStatus)) {
            ...
            int lChannelIndex =
                lCache->GetChannelIndex(lDeformer->Channel.Get());
            if (lCache->GetAnimationRange(lChannelIndex, lChannel_Start,
                                          lChannel_Stop)) {
...
// Load a texture file (TGA only now) into GPU and return the texture object
// name
bool LoadTextureFromFile(const FbxString& pFilePath,
  ...
  if (pFilePath.Right(3).Upper() == "TGA") {
    ...
    if (tga_read(&lTGAImage, pFilePath.Buffer()) == TGA_NOERR) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, 3, lTGAImage.width, lTGAImage.height, 0,
                   GL_BGR, GL_UNSIGNED_BYTE, lTGAImage.image_data);
...
void LoadCacheRecursive(FbxNode* pNode, FbxAnimLayer* pAnimLayer,
  ...
  for (int lMaterialIndex = 0; lMaterialIndex < lMaterialCount;
       ++lMaterialIndex) {
  ...
  for (int lMaterialIndex = 0; lMaterialIndex < lMaterialCount;
    ...
    if (lMaterial && lMaterial->GetUserDataPtr()) {
      MaterialCache* lMaterialCache =
          static_cast<MaterialCache*>(lMaterial->GetUserDataPtr());
    ...
    // Unload the light cache
      ...
      if (lLight && lLight->GetUserDataPtr()) {
        LightCache* lLightCache =
            static_cast<LightCache*>(lLight->GetUserDataPtr());
...
void LoadCacheRecursive(FbxScene* pScene, FbxAnimLayer* pAnimLayer,
                        const char* pFbxFileName, bool pSupportVBO) {
  ...
  for (int lTextureIndex = 0; lTextureIndex < lTextureCount; ++lTextureIndex) {
    ...
    if (lFileTexture && !lFileTexture->GetUserDataPtr()) {
      ...
      const FbxString lAbsFolderName =
          FbxPathUtils::GetFolderName(lAbsFbxFileName);
      if (!lStatus) {
        // Load texture from relative file name (relative to FBX file)
        const FbxString lResolvedFileName = FbxPathUtils::Bind(
            lAbsFolderName, lFileTexture->GetRelativeFileName());
      ...
      if (!lStatus) {
        ...
        const FbxString lResolvedFileName =
            FbxPathUtils::Bind(lAbsFolderName, lTextureFileName);
        lStatus = LoadTextureFromFile(lResolvedFileName, lTextureObject);
      }
...
// Unload the cache and release the memory fro this scene and release the
// textures in GPU
void UnloadCacheRecursive(FbxScene* pScene) {
  ...
  for (int lTextureIndex = 0; lTextureIndex < lTextureCount; ++lTextureIndex) {
    ...
    if (lFileTexture && lFileTexture->GetUserDataPtr()) {
      GLuint* lTextureName =
          static_cast<GLuint*>(lFileTexture->GetUserDataPtr());
...
bool InitializeOpenGL() {
  ...
  if (!GLEW_VERSION_1_5) {
    FBXSDK_printf(
        "The OpenGL version should be at least 1.5 to display shaded scene!\n");
    return false;
  }
...
SceneContext::SceneContext(const char* pFileName, int pWindowWidth,
                           int pWindowHeight, bool pSupportVBO)
    : mFileName(pFileName),
      mStatus(UNLOADED),
      mSdkManager(NULL),
      mScene(NULL),
  ...
  if (mSdkManager) {
    ...
    if (!mSdkManager->GetIOPluginRegistry()->DetectReaderFileFormat(
            mFileName, lFileFormat)) {
      // Unrecognizable file format. Try to fall back to
      // FbxImporter::eFBX_BINARY
      lFileFormat =
          mSdkManager->GetIOPluginRegistry()->FindReaderIDByDescription(
              "FBX binary (*.fbx)");
      ;
    }
  ...
  if (mStatus == MUST_BE_LOADED) {
    if (mImporter->Import(mScene) == true) {
      ...
      FbxAxisSystem SceneAxisSystem =
          mScene->GetGlobalSettings().GetAxisSystem();
      ...
      FbxSystemUnit SceneSystemUnit =
          mScene->GetGlobalSettings().GetSystemUnit();
...
bool SceneContext::SetCurrentAnimStack(int pIndex) {
  ...
  FbxAnimStack* lCurrentAnimationStack =
      mScene->FindMember<FbxAnimStack>(mAnimStackNameArray[pIndex]->Buffer());
  if (lCurrentAnimationStack == NULL) {
    // this is a problem. The anim stack should be found in the scene!
    return false;
  }

  // we assume that the first animation layer connected to the animation stack
  // is the base layer (this is the assumption made in the FBXSDK)
  ...
  FbxTakeInfo* lCurrentTakeInfo =
      mScene->GetTakeInfo(*(mAnimStackNameArray[pIndex]));
  if (lCurrentTakeInfo) {
  ...
  if (mCache_Start < mStart) mStart = mCache_Start;
...
void SceneContext::OnMouseMotion(int pX, int pY) {
  ...
  switch (mCameraStatus) {
    ...
    case CAMERA_PAN:
      CameraPan(mScene, mCamPosition, mCamCenter, mRoll, pX - mLastX,
                mLastY - pY);
      mStatus = MUST_BE_REFRESHED;
      break;
  }
