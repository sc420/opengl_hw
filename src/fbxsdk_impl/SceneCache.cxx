...
// Value = Property value * Factor property value (if no factor property,
// multiply by 1).
FbxDouble3 GetMaterialProperty(const FbxSurfaceMaterial *pMaterial,
  ...
  const FbxProperty lFactorProperty =
      pMaterial->FindProperty(pFactorPropertyName);
...
VBOMesh::VBOMesh()
    : mHasNormal(false), mHasUV(false), mAllByControlPoint(true) {
...
bool VBOMesh::Initialize(const FbxMesh *pMesh) {
  ...
  FbxGeometryElement::EMappingMode lMaterialMappingMode =
      FbxGeometryElement::eNone;
  if (pMesh->GetElementMaterial()) {
    lMaterialIndice = &pMesh->GetElementMaterial()->GetIndexArray();
    lMaterialMappingMode = pMesh->GetElementMaterial()->GetMappingMode();
    if (lMaterialIndice &&
        lMaterialMappingMode == FbxGeometryElement::eByPolygon) {
      ...
      if (lMaterialIndice->GetCount() == lPolygonCount) {
        ...
        // Make sure we have no "holes" (NULL) in the mSubMeshes table. This can
        // happen if, in the loop above, we resized the mSubMeshes by more than
        // one slot.
        for (int i = 0; i < mSubMeshes.GetCount(); i++) {
          if (mSubMeshes[i] == NULL) mSubMeshes[i] = new SubMesh;
        }
  ...
  // If normal or UV is by polygon vertex, record all vertex attributes by
  // polygon vertex.
  ...
  if (mHasNormal) {
    ...
    if (mHasNormal &&
        lNormalMappingMode != FbxGeometryElement::eByControlPoint) {
  ...
  unsigned int *lIndices =
      new unsigned int[lPolygonCount * TRIANGLE_VERTEX_COUNT];
  float *lNormals = NULL;
  if (mHasNormal) {
  ...
  if (mAllByControlPoint) {
    ...
    for (int lIndex = 0; lIndex < lPolygonVertexCount; ++lIndex) {
      ...
      lVertices[lIndex * VERTEX_STRIDE + 1] =
          static_cast<float>(lCurrentVertex[1]);
      lVertices[lIndex * VERTEX_STRIDE + 2] =
      ...
      if (mHasNormal) {
        ...
        lNormals[lIndex * NORMAL_STRIDE] =
            static_cast<float>(lCurrentNormal[0]);
        lNormals[lIndex * NORMAL_STRIDE + 1] =
            static_cast<float>(lCurrentNormal[1]);
        lNormals[lIndex * NORMAL_STRIDE + 2] =
  ...
  for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex) {
    ...
    for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT;
         ++lVerticeIndex) {
      const int lControlPointIndex =
          pMesh->GetPolygonVertex(lPolygonIndex, lVerticeIndex);

      if (mAllByControlPoint) {
        lIndices[lIndexOffset + lVerticeIndex] =
            static_cast<unsigned int>(lControlPointIndex);
      }
      // Populate the array with vertex attribute, if by polygon vertex.
      else {
        lIndices[lIndexOffset + lVerticeIndex] =
            static_cast<unsigned int>(lVertexCount);

        lCurrentVertex = lControlPoints[lControlPointIndex];
        lVertices[lVertexCount * VERTEX_STRIDE] =
            static_cast<float>(lCurrentVertex[0]);
        lVertices[lVertexCount * VERTEX_STRIDE + 1] =
            static_cast<float>(lCurrentVertex[1]);
        lVertices[lVertexCount * VERTEX_STRIDE + 2] =
            static_cast<float>(lCurrentVertex[2]);
        lVertices[lVertexCount * VERTEX_STRIDE + 3] = 1;

        if (mHasNormal) {
          pMesh->GetPolygonVertexNormal(lPolygonIndex, lVerticeIndex,
                                        lCurrentNormal);
          lNormals[lVertexCount * NORMAL_STRIDE] =
              static_cast<float>(lCurrentNormal[0]);
          lNormals[lVertexCount * NORMAL_STRIDE + 1] =
              static_cast<float>(lCurrentNormal[1]);
          lNormals[lVertexCount * NORMAL_STRIDE + 2] =
              static_cast<float>(lCurrentNormal[2]);
        }

        if (mHasUV) {
          bool lUnmappedUV;
          pMesh->GetPolygonVertexUV(lPolygonIndex, lVerticeIndex, lUVName,
                                    lCurrentUV, lUnmappedUV);
          lUVs[lVertexCount * UV_STRIDE] = static_cast<float>(lCurrentUV[0]);
          lUVs[lVertexCount * UV_STRIDE + 1] =
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               lPolygonCount * TRIANGLE_VERTEX_COUNT * sizeof(unsigned int),
               lIndices, GL_STATIC_DRAW);
  delete[] lIndices;

  return true;
}

void VBOMesh::UpdateVertexPosition(const FbxMesh *pMesh,
                                   const FbxVector4 *pVertices) const {
  ...
  if (mAllByControlPoint) {
    ...
    for (int lIndex = 0; lIndex < lVertexCount; ++lIndex) {
      lVertices[lIndex * VERTEX_STRIDE] =
          static_cast<float>(pVertices[lIndex][0]);
      lVertices[lIndex * VERTEX_STRIDE + 1] =
          static_cast<float>(pVertices[lIndex][1]);
      lVertices[lIndex * VERTEX_STRIDE + 2] =
          static_cast<float>(pVertices[lIndex][2]);
      lVertices[lIndex * VERTEX_STRIDE + 3] = 1;
    }
  } else {
    const int lPolygonCount = pMesh->GetPolygonCount();
    ...
    for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount;
      ...
      for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT;
        ...
        const int lControlPointIndex =
            pMesh->GetPolygonVertex(lPolygonIndex, lVerticeIndex);
        lVertices[lVertexCount * VERTEX_STRIDE] =
            static_cast<float>(pVertices[lControlPointIndex][0]);
        lVertices[lVertexCount * VERTEX_STRIDE + 1] =
            static_cast<float>(pVertices[lControlPointIndex][1]);
        lVertices[lVertexCount * VERTEX_STRIDE + 2] =
            static_cast<float>(pVertices[lControlPointIndex][2]);
        lVertices[lVertexCount * VERTEX_STRIDE + 3] = 1;
        ++lVertexCount;
      }
    }
  ...
  if (lVertices) {
    glBindBuffer(GL_ARRAY_BUFFER, mVBONames[VERTEX_VBO]);
    glBufferData(GL_ARRAY_BUFFER, lVertexCount * VERTEX_STRIDE * sizeof(float),
                 lVertices, GL_STATIC_DRAW);
    delete[] lVertices;
  }
}
...
#pragma warning(disable : 4312)
  ...
  // Where to start.
  GLsizei lOffset =
      mSubMeshes[pMaterialIndex]->IndexOffset * sizeof(unsigned int);
  if (pShadingMode == SHADING_MODE_SHADED) {
    ...
    for (int lIndex = 0; lIndex < mSubMeshes[pMaterialIndex]->TriangleCount;
         ++lIndex) {
...
bool MaterialCache::Initialize(const FbxSurfaceMaterial *pMaterial) {
  const FbxDouble3 lEmissive = GetMaterialProperty(
      pMaterial, FbxSurfaceMaterial::sEmissive,
      FbxSurfaceMaterial::sEmissiveFactor, mEmissive.mTextureName);
  ...
  const FbxDouble3 lAmbient = GetMaterialProperty(
      pMaterial, FbxSurfaceMaterial::sAmbient,
      FbxSurfaceMaterial::sAmbientFactor, mAmbient.mTextureName);
  ...
  const FbxDouble3 lDiffuse = GetMaterialProperty(
      pMaterial, FbxSurfaceMaterial::sDiffuse,
      FbxSurfaceMaterial::sDiffuseFactor, mDiffuse.mTextureName);
  ...
  const FbxDouble3 lSpecular = GetMaterialProperty(
      pMaterial, FbxSurfaceMaterial::sSpecular,
      FbxSurfaceMaterial::sSpecularFactor, mSpecular.mTextureName);
  ...
  FbxProperty lShininessProperty =
      pMaterial->FindProperty(FbxSurfaceMaterial::sShininess);
...
bool LightCache::Initialize(const FbxLight *pLight, FbxAnimLayer *pAnimLayer) {
  ...
  if (pAnimLayer) {
    mColorRed.mAnimCurve =
        lColorProperty.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COLOR_RED);
    mColorGreen.mAnimCurve =
        lColorProperty.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COLOR_GREEN);
    mColorBlue.mAnimCurve =
        lColorProperty.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COLOR_BLUE);
  }
...
void LightCache::SetLight(const FbxTime &pTime) const {
  const GLfloat lLightColor[4] = {mColorRed.Get(pTime), mColorGreen.Get(pTime),
                                  mColorBlue.Get(pTime), 1.0f};
...
void LightCache::IntializeEnvironment(const FbxColor &pAmbientLight) {
  ...
  // Set ambient light.
  GLfloat lAmbientLight[] = {static_cast<GLfloat>(pAmbientLight[0]),
                             static_cast<GLfloat>(pAmbientLight[1]),
