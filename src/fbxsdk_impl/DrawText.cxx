  ...
  while (*pCharacter != '\0') {
    if (*pCharacter == '\n') {
      ...
      if (lGlyph) {
        ...
        glVertex2f(lGlyph->vertex_left * mPointSize,
                   lGlyph->vertex_bottom * mPointSize);
        glTexCoord2f(lGlyph->texture_right, lGlyph->texture_bottom);
        glVertex2f(lGlyph->vertex_right * mPointSize,
                   lGlyph->vertex_bottom * mPointSize);
        glTexCoord2f(lGlyph->texture_right, lGlyph->texture_top);
        glVertex2f(lGlyph->vertex_right * mPointSize,
                   lGlyph->vertex_top * mPointSize);
...
void DrawText::Initialize() {
  ...
  if (fread(lTextureData, 1, lTextureSize, lFile) == lTextureSize) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, TEXTURE_WIDTH,
                 TEXTURE_HEIGHT, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,
                 lTextureData);
