...
/*
 ...
 http://www.johno.se/book/imgui.html or watch http://mollyrocket.com/861 See
 README.md for more links describing the IMGUI paradigm. Dear ImGui is an
 implementation of the IMGUI paradigm.
 ...
 source code. (Read the 'misc/fonts/README.txt' file for more details about font
 loading.)
 ...
 strings. (Read the 'misc/fonts/README.txt' file for more details about icons
 font loading.)
 ...
 them ImGui::GetDrawListSharedData(), or create your own ImDrawListSharedData,
      and then call your rendered code with your own ImDrawList or ImDrawData
 data.
 ...
 or docs/TODO.txt and see how you want/can help!
    - Convince your company to sponsor/fund development! Individual users: you
 can also become a Patron (patreon.com/imgui) or donate on PayPal! See README.
...
#pragma clang diagnostic ignored \
    "-Wsign-conversion"  // warning : implicit conversion changes
                         // signedness             //
#pragma clang diagnostic ignored \
...
ImGuiStyle::ImGuiStyle() {
  ...
  DisplayWindowPadding = ImVec2(
      20,
      20);  // Window position are clamped to be visible within the display area
...
void ImGui::EndFrame() {
  ...
  IM_ASSERT(g.CurrentWindowStack.Size ==
            1);  // Mismatched Begin()/End() calls, did you forget to call end
                 // on g.CurrentWindow->Name?
  ...
  ImVec2 padding_for_resize_from_edges =
      g.IO.ConfigResizeWindowsFromEdges
          ? ImMax(g.Style.TouchExtraPadding,
                  ImVec2(RESIZE_WINDOWS_FROM_EDGES_HALF_THICKNESS,
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
  ...
  if (g.IO.MouseDown[button])
    if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
      return g.IO.MousePos -
             g.IO.MouseClickedPos[button];  // Assume we can only get active
...
static void ImGui::UpdateManualResize(ImGuiWindow* window,
  ...
  if ((flags & ImGuiWindowFlags_NoResize) ||
      ...
      window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
    return;
  if (window->WasActive == false)  // Early out to avoid running this code for
                                   // e.g. an hidden implicit Debug window.
    return;
  ...
  for (int border_n = 0; border_n < resize_border_count; border_n++) {
    ...
    // IM_COL32(255, 255, 0, 255));
    if ((hovered &&
         g.HoveredIdTimer > RESIZE_WINDOWS_FROM_EDGES_FEEDBACK_TIMER) ||
        held) {
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window) {
  ...
  if (n <
      g.OpenPopupStack.Size)  // This test is not required but it allows to set
                              // a convenient breakpoint on the block below
    ClosePopupToLevel(n);
}
    ...
    if ((page_up_held && !page_down_held) ||
      ...
      if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
        ...
        if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
          SetWindowScrollY(
              window, window->Scroll.y - window->InnerClipRect.GetHeight());
